...
class GitHubMCPConnector(MCPConnector):
    ...
    async def execute_action(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.breaker.state == "OPEN":
            ...
            return {"error": "Circuit breaker is open due to repeated failures. Please try again later.", "action": action}
        ...
        if handler:
            try:
                ...
                # Successful API call, even if GitHub returns a logical error (e.g. "not found")
                # We check for success before resetting the breaker.
                if isinstance(result, dict) and result.get('success') is False:
                     # This indicates a logical failure (e.g. 404 Not Found), not necessarily a service failure
                     pass
                ...
                logger.error(f"An unexpected error occurred during action '{action}': {e}")
                self.breaker.record_failure()
                return {"success": False, "error": str(e), "action": action}
            ...
            async with self.session.get(url, params=params_dict) as response:
                if response.status == 200:
                    ...
                    repositories = [{
                        ...
                        'url': repo['html_url'], 'api_url': repo['url'], 'created_at': repo['created_at'], 'updated_at': repo['updated_at']
                    } for repo in data.get('items', [])]
                    return {'success': True, 'total_count': data.get('total_count', 0), 'repositories': repositories}
                else:
    ...
    async def get_issues(self, params: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            async with self.session.get(url, params={k: v for k, v in params_dict.items() if v}) as response:
                if response.status == 200:
    ...
    async def get_commits(self, params: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            params_dict = {
                'sha': params.get('sha', 'main'), 'per_page': params.get('per_page', 30),
    ...
    async def get_rate_limit(self, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    data = await response.json()
                    return {'success': True, 'rate_limit': data['resources']['core']}
                else:
...
async def demonstrate_github_connector():
    ...
    search_result = await github_connector.execute_action('search_repositories',{
    ...
    repo_result = await github_connector.execute_action('get_repository', {
