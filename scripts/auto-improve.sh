...
git add -A || { echo "❌ Error: Failed to add files to Git. Exiting..."; exit 1; }
if ! git diff --cached --quiet; then
  git commit -m "feat: prepare for CodeRabbit auto-improvements" || { echo "❌ Error: Failed to commit changes. Exiting..."; exit 1; }
fi

# 2. Push to trigger CodeRabbit review
echo "⬆️ Pushing to GitHub for CodeRabbit analysis..."
git push || { echo "❌ Error: Failed to push changes to GitHub. Exiting..."; exit 1; }
