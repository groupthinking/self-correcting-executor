...
# check_for_new_commits checks if the remote branch contains commits not present in the local HEAD and returns success if new commits are detected.
...
# wait_for_coderabbit waits for new commits from CodeRabbit by polling the remote repository until a commit is detected or a maximum wait time is reached. If no new commits are found within the maximum duration, it waits for a fallback period before proceeding.
wait_for_coderabbit() {
